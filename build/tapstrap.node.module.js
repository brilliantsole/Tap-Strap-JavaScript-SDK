...
class Timer {
    ...
    start(immediately = false) {
        ...
        _console$f.log("starting interval");
...
const _console$b = createConsole("bluetoothUUIDs", { log: true });
...
const _console$6 = createConsole("RawSensorUtils");
...
function assertValidRawSensorSensitivityForType(rawSensorType, index) {
    ...
    _console$6.assertWithError(value != undefined, `invalid RawSensorSensitivity index ${index} for sensor "${rawSensorType}" (got value ${value})`);
}
...
const _console$5 = createConsole("InputManager");
const InputModes = [
    "controller",
    "text",
    "rawSensor",
...
class InputManager {
    ...
    setSensitivityForType(rawSensorType, index) {
        assertValidRawSensorSensitivityForType(rawSensorType, index);
        _console$5.log(`setting ${rawSensorType} sensitivity index to ${index}`);
    ...
    set mode(newMode) {
        ...
        if (this.mode == newMode) {
            _console$5.log(`redundant mode assignment "${newMode}"`);
            return;
        }
...
_InputManager_sensitivity = new WeakMap(), _InputManager_mode = new WeakMap(), _InputManager_timer = new WeakMap(), _InputManager_instances = new WeakSet(), _InputManager_assertValidMode = function _InputManager_assertValidMode(mode) {
    _console$5.assertEnumWithError(mode, InputModes);
    ...
    if (this.mode == "rawSensor") {
        _console$5.assertWithError(this.sensitivity, "no sensitivity defined for rawSensor input mode");
    ...
    _console$5.log("sending mode data...");
...
export { Device, DeviceManager$1 as DeviceManager, environment as Environment, InputModes, MaxNumberOfVibrationSegments, MaxNumberOfVibrations, RangeHelper, RawSensorSensitivityFactors, RawSensorTypes, scanner$1 as Scanner, setAllConsoleLevelFlags, setConsoleLevelFlagsForType };
